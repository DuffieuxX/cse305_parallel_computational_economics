...
// #3b GPU order generation + CPU sequential market clearing
void add_all_new_orders_gpu(Market& market, Params& params, GpuParams& gpu_params, Agent* agents, Order* orders, Order_book& final_order_book){
    ...
    // 2. Use CPU threads as clearing batches
    ...
    // 3. Create one local book per batch
    ...
    // 4. Clear each batch locally in parallel
        ...
        batch_threads[b] = std::thread([&, b]() {
            ...
            for (int i = start; i < end; ++i) {
                if (orders[i].quantity != 0.0) {
                    ...
                    local_books[b].add_order(agents, new_order);
                }
            }
        });
    }
    ...
    // 5. Aggregate local traded volume into final_order_book
    for (int b = 0; b < nb_batches; ++b) {
        final_order_book.volume += local_books[b].volume;
        final_order_book.volume_weighted_sum += local_books[b].volume_weighted_sum;
    }

    // 6. Collect residual orders from all local books
    std::vector<Order*> residual_orders;
    residual_orders.reserve(params.N);

    for (int b = 0; b < nb_batches; ++b) {
        for (Order* o : local_books[b].bids) {
            residual_orders.push_back(o);
        }
        local_books[b].bids.clear();

        for (Order* o : local_books[b].asks) {
            residual_orders.push_back(o);
        }
        local_books[b].asks.clear();
    }

    // 7. Sequentially clear residual orders in one final global book
    // This reconciliation is sequential.
    for (Order* o : residual_orders) {
        if (o->quantity != 0.0) {
            final_order_book.add_order(agents, o);
        } else {
            delete o;
        }
    }
