...
void add_all_new_orders(std::vector<Market>& vector_markets, Params& params, std::vector<Agent*>& agents, std::vector<Order_book>& vector_order_books){
    int nb_threads =params.nb_markets;
    std::vector<std::thread> vector_threads(nb_threads);

    for(int asset_id=0; asset_id<nb_threads;asset_id++){

        vector_threads[asset_id]= std::thread(add_all_new_orders_per_market,std::ref(vector_markets[asset_id]),  std::ref(params), std::ref(agents), std::ref(vector_order_books[asset_id]));
    }
