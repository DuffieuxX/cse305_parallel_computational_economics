...
// #5 Update agents
//Parallel
void update_agents_thread(int index_start, int index_end, std::vector<Agent*>& agents, Probs& probs, unsigned int seed_thread, int& counter_optimist, int& counter_pessimist, int& counter_fundamentalist){
...
Counts update_agents( Params& params, std::vector<Agent*>& agents, Probs& probs, std::mt19937& rng ) {
    ...
    for (int i = 0; i < nb_threads; i++) {
        vector_threads[i].join();
        counts_after.nb_optimists += counter_optimist[i];
        counts_after.nb_pessimists += counter_pessimist[i];
        counts_after.nb_fundamentalists += counter_fundamentalist[i];
    }
...
std::vector<Order> generate_all_new_orders(
    ...
    for (int i = 0; i < nb_threads; i++) {
        int index_start = i * chunk_size;
        int index_end = std::min(index_start + chunk_size, params.N);

        vector_threads[i] = std::thread(
            add_all_new_orders_thread,
            index_start,
            index_end,
            std::ref(market),
            std::ref(params),
            std::ref(agents),
            std::ref(vector_new_orders_thread[i])
        );
    }
    ...
    for (int i = 0; i < nb_threads; i++) {
        ...
        all_orders.insert(
            all_orders.end(),
            vector_new_orders_thread[i].begin(),
            vector_new_orders_thread[i].end()
        );
    }
...
//Parallel
// Simulation loop
// Parallel version, benchmark-compatible
SimTimes run_simulation(Params& params) {
    ...
    // Ceiling division: covers all agents even when N is not divisible by nb_threads
    ...
    double time_order_generation = 0.0;
    double time_clearing = 0.0;
    double time_price_update = 0.0;
    ...
    for (int t = 0; t < params.T; ++t) {
        ...
        time_counting += std::chrono::duration<double, std::milli>(
            std::chrono::high_resolution_clock::now() - t_count
        ).count();

        // 2. Compute transition probabilities
        auto t_prob = std::chrono::high_resolution_clock::now();

        Probs probs = compute_probabilities(params, market, counts_before);

        time_probabilities += std::chrono::duration<double, std::milli>(
            std::chrono::high_resolution_clock::now() - t_prob
        ).count();
        ...
        time_updating += std::chrono::duration<double, std::milli>(
            std::chrono::high_resolution_clock::now() - t_update
        ).count();

        /*
            4. Order generation + clearing

            In your current implementation, add_all_new_orders() does both:
            - parallel order generation;
            - sequential order-book insertion / clearing.

            So for now, we measure the whole thing as "clearing",
            because the function does not separate generation from clearing yet.

            If you later split add_all_new_orders() into:
                generate_all_new_orders(...)
                clear_all_orders(...)
            then you can measure order_generation and clearing separately.
        */
        auto t_order = std::chrono::high_resolution_clock::now();

        std::vector<Order> orders = generate_all_new_orders(market, params, agents);

        time_order_generation += std::chrono::duration<double, std::milli>(
            std::chrono::high_resolution_clock::now() - t_order
        ).count();

        auto t_clear = std::chrono::high_resolution_clock::now();

        clear_all_orders(agents, order_book, orders);

        time_clearing += std::chrono::duration<double, std::milli>(
            std::chrono::high_resolution_clock::now() - t_clear
        ).count();
        ...
        // 5. Price update
        auto t_price = std::chrono::high_resolution_clock::now();

        double new_price = update_price(order_book);

        time_price_update += std::chrono::duration<double, std::milli>(
            std::chrono::high_resolution_clock::now() - t_price
        ).count();
        ...
        if (params.write_output) {
            ...
            time_output += std::chrono::duration<double, std::milli>(
                std::chrono::high_resolution_clock::now() - t_out
            ).count();
        }
    ...
    simtimes.probabilities = time_probabilities;
    simtimes.updating = time_updating;
    simtimes.order_generation = time_order_generation;
    simtimes.clearing = time_clearing;
    simtimes.price_update = time_price_update;
    simtimes.output = time_output;
