...
// #6 New orders
// V4 logic:
// - each thread builds and clears a local order book for its batch;
// - then remaining orders from local books are sequentially inserted into the global book;
// - local traded volume is added to the global volume.

void add_all_new_orders_thread(
    int index_start,
    int index_end,
    const Market& market,
    const Params& params,
...
void add_all_new_orders(
    ...
    std::vector<std::thread> vector_threads(nb_threads);
    std::vector<Order_book> vector_local_order_books;

    vector_local_order_books.reserve(nb_threads);
    ...
    for (int i = 0; i < nb_threads; i++) {
        ...
        vector_local_order_books.emplace_back(params, nb_agents_in_batch);
    }

    auto t_parallel_batches = std::chrono::high_resolution_clock::now();
    ...
    for (int i = 0; i < nb_threads; i++) {
        ...
        vector_threads[i] = std::thread(
            ...
            std::ref(params),
            std::ref(agents),
            std::ref(vector_local_order_books[i])
        );
    }
    ...
    simtimes.order_generation += std::chrono::duration<double, std::milli>(
        std::chrono::high_resolution_clock::now() - t_parallel_batches
    ).count();

    auto t_sequential_clearing = std::chrono::high_resolution_clock::now();

    for (int i = 0; i < nb_threads; i++) {
        for (const Order& new_order : vector_local_order_books[i].order_storage) {
            order_book.add_order(agents, new_order);
        }

        order_book.volume += vector_local_order_books[i].volume;
        order_book.volume_weighted_sum += vector_local_order_books[i].volume_weighted_sum;
    }

    simtimes.clearing += std::chrono::duration<double, std::milli>(
        std::chrono::high_resolution_clock::now() - t_sequential_clearing
    ).count();
}
...
//Parallel
// Simulation loop
// Parallel benchmark-compatible version
// Simulation loop
// Parallel V4 benchmark-compatible version
...
SimTimes run_simulation(Params& params) {
    ...
    for (int t = 0; t < params.T; ++t) {
        ...
        auto t_count_0 = std::chrono::high_resolution_clock::now();

        Counts counts_before = count_agents(agents, params);

        simtimes.counting += std::chrono::duration<double, std::milli>(
            std::chrono::high_resolution_clock::now() - t_count_0
        ).count();

        auto t_prob_0 = std::chrono::high_resolution_clock::now();

        Probs probs = compute_probabilities(params, market, counts_before);

        simtimes.probabilities += std::chrono::duration<double, std::milli>(
            std::chrono::high_resolution_clock::now() - t_prob_0
        ).count();

        auto t_update_0 = std::chrono::high_resolution_clock::now();

        update_agents(params, agents, probs, rng);

        simtimes.updating += std::chrono::duration<double, std::milli>(
            std::chrono::high_resolution_clock::now() - t_update_0
        ).count();

        auto t_count_1 = std::chrono::high_resolution_clock::now();

        Counts counts_after = count_agents(agents, params);

        simtimes.counting += std::chrono::duration<double, std::milli>(
            std::chrono::high_resolution_clock::now() - t_count_1
        ).count();
        ...
        auto t_output_0 = std::chrono::high_resolution_clock::now();
        ...
        simtimes.output += std::chrono::duration<double, std::milli>(
            std::chrono::high_resolution_clock::now() - t_output_0
        ).count();
